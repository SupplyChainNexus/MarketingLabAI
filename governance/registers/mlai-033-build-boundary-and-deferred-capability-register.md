...
| Evidence | RFC 8785-compatible restricted-number `MLAI-CJ-1` canonical UTF-8 JSON with exact request, receipt and evidence envelopes and domains, privacy-safe fields, domain-separated SHA-256, sequence-1 genesis, append-only ordering and hash linking committed atomically with authority-changing state; retention and archival remain unresolved. |
...
| Canonical artifact persistence | ADR-0043 now defines the read-only availability proof, but generated content and compliance outputs still lack the canonical repository that can issue it. | Before any workflow may enter `running`, its artifact reference resolves to an accessible, tenant-owned, non-superseded reserved or persisted target; before `completed`, publishing or learning evidence, it resolves to a persisted integrity proof. The repository and recovery contract must pass SQLite/PostgreSQL compatibility and cross-tenant tests. | Architecture authority plus separate IMPLEMENT, TEST and persistence-migration authority. | The interface contract is governance only; no artifact or workflow implementation is authorized. |
