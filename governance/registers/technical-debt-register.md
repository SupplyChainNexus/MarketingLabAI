...
| TD-040 | Repository integrity heritage | The 18A baseline acknowledges inherited encoding, line-ending, governance-structure and legacy-manifest findings so current integrity checks can distinguish known debt from new regressions. | High | Retire findings in small exact-file, exact-current-hash and exact-replacement-hash plans; update the baseline only through reviewed governance work and never through implicit bulk rewriting. |
